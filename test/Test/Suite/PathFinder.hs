...
length1Tests = TestList
  [ TestCase $ do
      let paths = findPathsLength1 (AtomId "свобода")
  ...
  , TestCase $ do
      let paths = findPathsLength1 (AtomId "несуществующий_топик")
      assertEqual "nonexistent topic has 0 paths" 0 (length paths)

  , TestCase $ do
      let paths = findPathsLength1 (AtomId "истина")
...
length2Tests = TestList
  [ TestCase $ do
      let paths = findPathsLength2 (AtomId "свобода")
...
length3Tests = TestList
  [ TestCase $ do
      let paths = findPathsLength3 (AtomId "свобода")
...
fieldBiasTests = TestList
  ...
  , TestCase $ do
      let highConf = FieldProfile 1.0 0.0 0.0 0.0
          paths = selectTopPaths 3 highConf (AtomId "истина") 1
          allPaths = findPathsLength1 (AtomId "истина")
  ...
  , TestCase $ do
      let highConf = FieldProfile 1.0 0.0 0.0 0.0
          ...
          confPaths = selectTopPaths 1 highConf (AtomId "истина") 1
          consolPaths = selectTopPaths 1 highConsolid (AtomId "истина") 1
...
determinismTests = TestList
  [ TestCase $ do
      let p1 = findPathsFrom 2 (AtomId "свобода")
          p2 = findPathsFrom 2 (AtomId "свобода")
      assertEqual "same input → same output (findPathsFrom)" p1 p2

  , TestCase $ do
      let p1 = selectTopPaths 3 defaultFieldProfile (AtomId "истина") 2
          p2 = selectTopPaths 3 defaultFieldProfile (AtomId "истина") 2
      assertEqual "same input → same output (selectTopPaths)" p1 p2

  , TestCase $ do
      let p1 = rankPaths (findPathsLength1 (AtomId "свобода"))
          p2 = rankPaths (findPathsLength1 (AtomId "свобода"))
...
compositionTests = TestList
  [ TestCase $ do
      let text = composeDefinition defaultFieldProfile 3 (AtomId "свобода")
      assertBool ("composeDefinition produces non-empty text: " <> show (T.length text))
                 (T.length text > 0)

  , TestCase $ do
      let text = composeDefinition defaultFieldProfile 3 (AtomId "свобода")
      assertBool ("composed text contains 'свобода': " <> show text)
                 ("свобода" `T.isInfixOf` text)

  , TestCase $ do
      let text = composeDefinition defaultFieldProfile 3 (AtomId "истина")
      assertBool ("composed text contains 'истина': " <> show text)
                 ("истина" `T.isInfixOf` text)

  , TestCase $ do
      let text = composeDefinition defaultFieldProfile 10 (AtomId "несуществующий")
      assertEqual "nonexistent topic produces empty text" "" text
  ]
