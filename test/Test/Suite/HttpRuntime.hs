    ...
    withRuntimeEnv "qxfx0_test_http_session_token_restart.db" $
      withEnvVar "QXFX0_API_KEY" (Just "test-api-key") $ do
        ...
        withSidecarOnPort port2 [] $ do
          ...
          (reclaimCode, reclaimValue) <- postTurnRawAuthenticated port2 "test-api-key" Nothing "persisted-owner" "После restart без token"
          assertEqual "graceful sidecar restart should clear stale ownership and allow fresh claim" 200 reclaimCode
          newToken <- requireTextField "restart reclaim payload must include fresh session token" "session_token" reclaimValue
          assertBool "graceful restart must rotate session token ownership" (newToken /= token)
          (oldCode, oldValue) <- postTurnRawAuthenticated port2 "test-api-key" (Just token) "persisted-owner" "После restart со старым token"
          assertEqual "stale session token must not survive graceful restart" 403 oldCode
          oldErr <- requireTextField "stale restart token payload" "error" oldValue
          assertEqual "stale restart token error must stay explicit" "invalid_session_token" oldErr
        ...
        withEnvVar "QXFX0_HTTP_HOST" (Just "0.0.0.0") $
          withEnvVar "QXFX0_HOST" Nothing $
            withEnvVar "QXFX0_ALLOW_NON_LOOPBACK_HTTP" Nothing $ do
              binPath <- resolveQxFx0MainBinary
              port <- allocatePort
              let args = ["--serve-http", "--port", show port, "--bin", binPath]
              timed <- timeout (10 * 1000000) (readProcessWithExitCode binPath args "")
              case timed of
                Nothing ->
...
startPortOccupier port = do
  pythonBin <- requireTrustedPython
  createProcess
    (proc pythonBin
      [ "-c"
