...
-- | P1.8: Verify that batch 153-182 concepts are curated
testCuratedPredicatesBatch153To182 :: Test
testCuratedPredicatesBatch153To182 = TestLabel "curated predicates batch 153-182" $ TestCase $ do
  curated <- loadCuratedPredicates curatedPredicatesPath
  let batch153To182 = [ "открытость пересмотру", "открытость проверке"
                      , "отсутствие движения и напряжения", "отсутствие значимого другого"
                      , "отсутствие контакта", "отсутствие основания"
                      , "отсутствие принуждения", "отсутствием движения"
                      , "отсутствием слов", "память", "память коллектива"
                      , "паралич или действие", "пауза в действии", "пауза и смысл"
                      , "переводимость и непереводимость", "перед неизвестным"
                      , "переживание значимости", "пережитое в новой рамке"
                      , "перспектива наблюдателя", "повторное переживание"
                      , "позиция субъекта", "покой", "порядок мысли"
                      , "порядок следования событий", "поток переживаний"
                      , "потребность в другом", "потребностью и распределением ресурсов"
                      , "почему а не зачем", "поэзия", "правда"
                      ]
      missing = filter (\t -> not (M.member t curated)) batch153To182
  assertEqual "all batch 153-182 topics must be present in curated predicates"
    [] missing
  mapM_ (\t ->
           case M.lookup t curated of
             Nothing -> assertFailure ("batch 153-182 topic " ++ T.unpack t ++ " missing")
             Just dc -> assertBool ("batch 153-182 topic " ++ T.unpack t ++ " must have >=2 predicates")
                                   (length (dcPredicates dc) >= 2))
        batch153To182

-- | P1.9: Verify that batch 183-212 concepts are curated
testCuratedPredicatesBatch183To212 :: Test
testCuratedPredicatesBatch183To212 = TestLabel "curated predicates batch 183-212" $ TestCase $ do
  curated <- loadCuratedPredicates curatedPredicatesPath
  let batch183To212 = [ "право", "праздность созерцания"
                      , "превосходство над полезным", "предел действия"
                      , "предел слова", "предел существования"
                      , "пределы контроля", "преобразование материи"
                      , "преодоление сопротивления", "преодоление эгоцентризма"
                      , "преодоления препятствий и конкурирующих мотивов"
                      , "привычка и рутина", "признание прав другого"
                      , "признание ценности другого", "принуждение и авторитет"
                      , "принятия без полного доказательства", "приоритет над желанием"
                      , "присутствие", "присутствие без выражения"
                      , "причина и следствие", "проверка опытом"
                      , "прозрачность намерений", "произвол"
                      , "пространство возможностей", "против забвения"
                      , "процедура и результат", "прошлое для настоящего"
                      , "прошлое с настоящим через интерпретацию"
                      , "прыжок за горизонт", "псевдосвобода"
                      ]
      missing = filter (\t -> not (M.member t curated)) batch183To212
  assertEqual "all batch 183-212 topics must be present in curated predicates"
    [] missing
  mapM_ (\t ->
           case M.lookup t curated of
             Nothing -> assertFailure ("batch 183-212 topic " ++ T.unpack t ++ " missing")
             Just dc -> assertBool ("batch 183-212 topic " ++ T.unpack t ++ " must have >=2 predicates")
                                   (length (dcPredicates dc) >= 2))
        batch183To212

-- | P1.10: Verify that batch 213-242 concepts are curated
testCuratedPredicatesBatch213To242 :: Test
testCuratedPredicatesBatch213To242 = TestLabel "curated predicates batch 213-242" $ TestCase $ do
  curated <- loadCuratedPredicates curatedPredicatesPath
  let batch213To242 = [ "психика", "пустота или вектор"
                       , "пустота как полнота", "равенство перед правилом"
                       , "разделение и кооперация"
                       , "различение истинного и ложного", "различение починить и заменить"
                       , "различение с истиной", "различение сущего и ничто"
                       , "различение я и не я", "разрушение порядка"
                       , "разрыв и восстановление", "разрыв и восстановление доверия"
                       , "разум", "рамка критериев", "революция"
                       , "рефлексия над собой", "решимость"
                       , "риск уязвимости", "рынок"
                       , "с речью, но не тождественно пустоте", "сам факт существования"
                       , "самокоррекция", "самооценка", "самосознание"
                       , "свидетельство", "свобода", "свобода и необходимость"
                       , "свобода как условие", "связь времён"
                       ]
      missing = filter (\t -> not (M.member t curated)) batch213To242
  assertEqual "all batch 213-242 topics must be present in curated predicates"
    [] missing
  mapM_ (\t ->
           case M.lookup t curated of
             Nothing -> assertFailure ("batch 213-242 topic " ++ T.unpack t ++ " missing")
             Just dc -> assertBool ("batch 213-242 topic " ++ T.unpack t ++ " must have >=2 predicates")
                                   (length (dcPredicates dc) >= 2))
        batch213To242

-- | P1.11: Verify that batch 243-272 concepts are curated
testCuratedPredicatesBatch243To272 :: Test
testCuratedPredicatesBatch243To272 = TestLabel "curated predicates batch 243-272" $ TestCase $ do
  curated <- loadCuratedPredicates curatedPredicatesPath
  let batch243To272 = [ "сжимает мир до значения", "сигнал опасности"
                       , "след прошлого", "слушание", "смерть"
                       , "собственность", "собственные состояния субъекта"
                       , "совесть как свидетель", "совпадение с собой"
                       , "созерцание и присутствие", "сознание"
                       , "соответствие реальности", "соразмерности между деянием и воздаянием"
                       , "соразмерность частей", "способность влиять на действия других"
                       , "способность к обобщению", "способность к самоотчёту"
                       , "справедливость", "страх", "страх и принятие"
                       , "структура значения", "субъекта объектом для самого себя"
                       , "темп и ритм", "тишина и пустота", "труд"
                       , "узнавание", "уравнивание всех"
                       , "урок и предупреждение", "усилие и результат"
                       , "усилие против энтропии"
                       ]
      missing = filter (\t -> not (M.member t curated)) batch243To272
  assertEqual "all batch 243-272 topics must be present in curated predicates"
    [] missing
  mapM_ (\t ->
           case M.lookup t curated of
             Nothing -> assertFailure ("batch 243-272 topic " ++ T.unpack t ++ " missing")
             Just dc -> assertBool ("batch 243-272 topic " ++ T.unpack t ++ " must have >=2 predicates")
                                   (length (dcPredicates dc) >= 2))
        batch243To272
...
curatedPredicatesTests =
  ...
  , testCuratedPredicatesBatch153To182
