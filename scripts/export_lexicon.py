...
ARGUED_LEAF_SLOTS = GF_DIR / "argued_leaf_slots.tsv"
...
OUT_HS_SEMANTIC_SLOTS = ROOT / "src" / "QxFx0" / "Lexicon" / "Generated" / "SemanticSlots.hs"
...
@dataclass(frozen=True)
class ArguedLeafSlot:
    leaf_id: str
    leaf_kind: str
    source_ru: str


def load_curated_predicate_slots() -> List[CuratedPredicateSlots]:
    """Load explicit predicate slots; no runtime surface parsing is permitted."""
    if not CURATED_PREDICATE_SLOTS.exists():
        raise ValueError(f"missing curated predicate slot source: {CURATED_PREDICATE_SLOTS}")
    rows: List[CuratedPredicateSlots] = []
    seen_ids: Set[str] = set()
    seen_surfaces: Set[str] = set()
    with CURATED_PREDICATE_SLOTS.open(encoding="utf-8") as handle:
        data_lines = [line.rstrip("\n") for line in handle if line.strip() and not line.startswith("#")]
    expected_header = ["predicate_id", "subject_id", "subject_ru", "relation_id", "relation_ru", "object_id", "object_ru", "source_ru"]
    if not data_lines or data_lines[0].split("\t") != expected_header:
        raise ValueError("invalid curated predicate slot source")
    for line in data_lines[1:]:
        fields = line.split("\t")
        if len(fields) != len(expected_header) or any(not field.strip() for field in fields):
            raise ValueError(f"invalid curated predicate slot row: {line}")
        entry = CuratedPredicateSlots(*fields)
        if entry.predicate_id in seen_ids or entry.source_ru in seen_surfaces:
            raise ValueError(f"duplicate curated predicate slot: {entry.predicate_id}")
        seen_ids.add(entry.predicate_id)
        seen_surfaces.add(entry.source_ru)
        rows.append(entry)
    return rows


def load_argued_leaf_slots() -> List[ArguedLeafSlot]:
    """Load exact, marker-free argued leaves for typed GF propositions."""
    if not ARGUED_LEAF_SLOTS.exists():
        raise ValueError(f"missing argued leaf slot source: {ARGUED_LEAF_SLOTS}")
    rows: List[ArguedLeafSlot] = []
    seen_ids: Set[str] = set()
    seen_surfaces: Set[str] = set()
    with ARGUED_LEAF_SLOTS.open(encoding="utf-8") as handle:
        data_lines = [line.rstrip("\n") for line in handle if line.strip() and not line.startswith("#")]
    expected_header = ["leaf_id", "leaf_kind", "source_ru"]
    if not data_lines or data_lines[0].split("\t") != expected_header:
        raise ValueError("invalid argued leaf slot source")
    for line in data_lines[1:]:
        fields = line.split("\t")
        if len(fields) != len(expected_header) or any(not field.strip() for field in fields):
            raise ValueError(f"invalid argued leaf slot row: {line}")
        entry = ArguedLeafSlot(*fields)
        if entry.leaf_kind not in {"counter", "synthesis"}:
            raise ValueError(f"invalid argued leaf kind: {entry.leaf_kind}")
        if entry.leaf_id in seen_ids or entry.source_ru in seen_surfaces:
            raise ValueError(f"duplicate argued leaf slot: {entry.leaf_id}")
...
def gf_slot_constructor(prefix: str, slot_id: str) -> str:
    return prefix + "".join(part.capitalize() for part in slot_id.split("_"))


def argued_leaf_constructor(entry: ArguedLeafSlot) -> str:
    return gf_slot_constructor("Argued" + entry.leaf_kind.capitalize(), entry.leaf_id)


def render_gf_syntax_files(
    predicates: List[CuratedPredicateSlots], argued_leaves: List[ArguedLeafSlot]
) -> Tuple[str, str]:
    subjects = {entry.subject_id: entry.subject_ru for entry in predicates}
    relations = {entry.relation_id: entry.relation_ru for entry in predicates}
    objects = {entry.object_id: entry.object_ru for entry in predicates}
    abstract_lines = [
        "-- AUTO-GENERATED by scripts/export_lexicon.py from spec/sql/lexicon.",
    ...
    abstract_lines.extend(
        [f"    {gf_slot_constructor('Subject', key)} : SemanticSubject ;" for key in sorted(subjects)]
    )
    abstract_lines.extend(
        [f"    {gf_slot_constructor('Relation', key)} : SemanticRelation ;" for key in sorted(relations)]
    )
    abstract_lines.extend(
        [f"    {gf_slot_constructor('Object', key)} : SemanticObject ;" for key in sorted(objects)]
    )
    abstract_lines.extend(
        [f"    {argued_leaf_constructor(entry)} : SemanticProposition ;" for entry in argued_leaves]
    )
    abstract_lines.append("}")
    ...
    concrete_lines.extend(
        [f"    {gf_slot_constructor('Subject', key)} = {{ s = {gf_quote(value)} }} ;" for key, value in sorted(subjects.items())]
    )
    concrete_lines.extend(
        [f"    {gf_slot_constructor('Relation', key)} = {{ s = {gf_quote(value)} }} ;" for key, value in sorted(relations.items())]
    )
    concrete_lines.extend(
        [f"    {gf_slot_constructor('Object', key)} = {{ s = {gf_quote(value)} }} ;" for key, value in sorted(objects.items())]
    )
    concrete_lines.extend(
        [f"    {argued_leaf_constructor(entry)} = {{ s = {gf_quote(entry.source_ru)} }} ;" for entry in argued_leaves]
    )
    concrete_lines.append("}")
...
def render_haskell_semantic_slots(
    predicates: List[CuratedPredicateSlots], argued_leaves: List[ArguedLeafSlot]
) -> str:
    out = [
    ...
    for index, entry in enumerate(predicates):
        ...
        out.append(
            "    CuratedPredicateSlots "
            f"{hs_quote(entry.source_ru)} "
            f"{hs_quote(gf_slot_constructor('Subject', entry.subject_id))} "
            f"{hs_quote(gf_slot_constructor('Relation', entry.relation_id))} "
            f"{hs_quote(gf_slot_constructor('Object', entry.object_id))}{suffix}"
        )
    out.extend(["  ]", "", "arguedLeafSlots :: [ArguedLeafSlot]", "arguedLeafSlots =", "  ["])
    for index, entry in enumerate(argued_leaves):
        suffix = "," if index < len(argued_leaves) - 1 else ""
        out.append(
            "    ArguedLeafSlot "
            f"{hs_quote(entry.source_ru)} "
            f"{hs_quote(argued_leaf_constructor(entry))}{suffix}"
        )
    out.extend(["  ]", ""])
    return "\n".join(out)
...
def main() -> int:
    ...
    curated_predicate_slots = load_curated_predicate_slots()
    argued_leaf_slots = load_argued_leaf_slots()
    gf_syntax_abstract, gf_syntax_concrete = render_gf_syntax_files(
        curated_predicate_slots, argued_leaf_slots
    )
    agda_data, agda_proof = render_agda_modules(rows)
    hs_runtime = render_haskell_runtime_module(rows, forms_by_surface)
    hs_semantic_slots = render_haskell_semantic_slots(curated_predicate_slots, argued_leaf_slots)
    ...
    quality = {
        ...
        "sources": [
            ...
            str(CURATED_PREDICATE_SLOTS.relative_to(ROOT)),
            str(ARGUED_LEAF_SLOTS.relative_to(ROOT)),
        ]
        + (
        ...
        "generated_artifacts": [
            ...
            str(OUT_HS_SEMANTIC_SLOTS.relative_to(ROOT)),
        ],
    }
    ...
    expected_outputs = {
        ...
        OUT_HS_SEMANTIC_SLOTS: hs_semantic_slots,
    }
    ...
    OUT_HS_SEMANTIC_SLOTS.parent.mkdir(parents=True, exist_ok=True)
