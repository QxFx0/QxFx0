...
-}
module QxFx0.Core.ContentCluster
  ( laplacianMatrix
  , fiedlerValue
  , fiedlerVector
  , detectClusters
  , ClusterInsight(..)
  , contentSalienceActive
  , computeContentSaliency
  ) where
...
allNodes adj =
  -- WP-C R-C2: deterministic node order via sort (tie-break by node id)
  sort $ M.keys (M.union adj (M.unions (map (\m -> M.fromList [(k, M.empty) | k <- M.keys m]) (M.elems adj))))
...
-- | WP-C: default-on promotion flag gating content-saliency computation from
-- spectral clustering. Promoted to default-on (2026-06-04); registered
-- in the flag-off discipline.
contentSalienceActive :: Bool
contentSalienceActive = True

-- | WP-C: compute a content-saliency signal from the meaning graph by counting
-- distinct spectral clusters. Returns @0.0@ when there are too few nodes to
-- cluster, else @min 1.0 (clusterCount / 10.0)@ — a normalised measure of how
-- many distinct topic regions the current dialogue graph has. Used as a top-
-- down weight into the Salience controller (increment 2).
computeContentSaliency :: MeaningGraph -> Double
computeContentSaliency mg
  | contentSalienceActive =
      let clusters = detectClusters mg
          n :: Int
          n        = case clusters of
                       NoClusters        -> 1
                       TwoClusters _ _ _ -> 2
          totalNodes = mgTurnCount mg
      in if totalNodes < 3 then 0.0 else min 1.0 (fromIntegral n / 10.0)
  | otherwise = 0.0
