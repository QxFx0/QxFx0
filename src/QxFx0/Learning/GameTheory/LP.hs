       ...
       in case final of
           ...
           Just (_, objR, objR_, bas, rhs_) ->
             let -- Primal variables w_i = - (objective coefficient of slack s_i)
                 w = [ let coeff = if (m + i) `elem` bas then 0.0 else objR !! (m + i)
                       in max 0.0 (-coeff)
                     | i <- [0 .. n-1] ]
                 total = sum w
             in if total > epsilon
           ...
           Just e  ->
             let -- Ratio test: for each row where coeff > 0, RHS / coeff
                 ratios = [ (conRHS !! i, i)
                          ...
                          , let coeff = conRows !! i !! e
                          , coeff > epsilon ]
             in if null ratios
                then Nothing  -- unbounded
                else let (_, l) = minimumBy (comparing fst) ratios
                         -- Pivot on row l, column e
                         pivotElem = conRows !! l !! e
                         pivotRow = map (/ pivotElem) (conRows !! l)
                         pivotRHS = conRHS !! l / pivotElem
                         -- Update all other rows
                         updateRow i row rhs =
                           let coeff = row !! e
                               newRow = zipWith (-) row (map (* coeff) pivotRow)
                               newRHS = rhs - coeff * pivotRHS
                           in (take e newRow ++ [0] ++ drop (e+1) newRow, newRHS)
                         (conRows', conRHS') = unzip
                           [ if i == l
                             then (pivotRow, pivotRHS)
                             else updateRow i (conRows !! i) (conRHS !! i)
                           | i <- [0 .. length conRows - 1] ]
                         -- Update objective row
                         objCoeffE = objRow !! e
                         newObjRow = zipWith (-) objRow (map (* objCoeffE) pivotRow)
                         newObjRHS = objRHS - objCoeffE * pivotRHS
                         newObjRow' = take e newObjRow ++ [0] ++ drop (e+1) newObjRow
...
findEntering objRow nonBasic =
  let candidates = filter (\j -> objRow !! j > epsilon) nonBasic
