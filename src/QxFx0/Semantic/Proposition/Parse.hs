...
-- Import detect modules
import QxFx0.Semantic.Proposition.Detect.Contact (detectContactSignal, detectAffectiveSupport)
import QxFx0.Semantic.Proposition.Detect.Ground
...
import QxFx0.Semantic.Proposition.Detect.Define
  ( detectConceptKnowledge
  , detectDistinctionQuestion
  )
import QxFx0.Semantic.Proposition.Detect.Reflect
  ( detectSelfKnowledge
  , detectSelfState
  )
import QxFx0.Semantic.Proposition.Detect.Repair
  ( detectRepairDirective
  , detectMisunderstanding
  )
import QxFx0.Semantic.Proposition.Detect.NextStep (detectNextStepSignal)
import QxFx0.Semantic.Proposition.Detect.Comparison (detectComparisonPlausibility)
import QxFx0.Semantic.Proposition.Detect.Overrides
...
-- Note: detectPurposeFunction, detectDialogueInvitation, detectConfrontSignal, detectGenerativePrompt, detectContemplativeTopic are not in Detect modules yet.
-- We'll add them here temporarily, but should be moved to appropriate Detect modules later.
...
followsFocusMarker key tokens =
  any (\(marker, value) -> marker `elem` focusMarkers && value == key) (zip tokens (drop 1 tokens))
