...
module QxFx0.Semantic.Network.RuntimeProjection
...
ensureRuntimeProjectionSchema db = do
  result <- withDB (qdbPath db) $ \conn -> do
    schema <- prepareTx conn "ensure_runtime_projection_schema"
      "CREATE TABLE IF NOT EXISTS semantic_edges_runtime (id INTEGER PRIMARY KEY AUTOINCREMENT, ts INTEGER NOT NULL, edge_from TEXT NOT NULL, edge_to TEXT NOT NULL, weight REAL NOT NULL, co_occurrence INTEGER NOT NULL, relation_type TEXT, domain TEXT, temporal_scope TEXT, verb TEXT, rationale TEXT, lineage TEXT, confidence REAL NOT NULL, provenance TEXT NOT NULL, namespace TEXT NOT NULL DEFAULT 'session_local')"
    stepOrFail schema
    -- Migration: add lineage column to tables created before 2026-07-13.
    migResult <- NSQL.execSql conn "ALTER TABLE semantic_edges_runtime ADD COLUMN lineage TEXT"
    case migResult of
      Right () -> pure (Right ())
      Left err
        | "duplicate column name" `T.isInfixOf` T.toLower err -> pure (Right ())
        | otherwise -> pure (Left err)
    idxEdge <- prepareTx conn "ensure_runtime_projection_idx_edge"
      "CREATE INDEX IF NOT EXISTS idx_runtime_edge ON semantic_edges_runtime(edge_from, edge_to)"
    stepOrFail idxEdge
...
persistRuntimeEdge :: QxFx0DB -> SemanticEdge -> IO ()
persistRuntimeEdge db edge = persistRuntimeEdges db [edge]

persistRuntimeEdges :: QxFx0DB -> [SemanticEdge] -> IO ()
persistRuntimeEdges _ [] = pure ()
persistRuntimeEdges db edges = do
  result <- withDB (qdbPath db) $ \conn -> do
    _ <- NSQL.execSql conn "BEGIN TRANSACTION;"
    now <- getCurrentTime
    mapM_ (insertOneEdge conn now) edges
    _ <- NSQL.execSql conn "COMMIT;"
    pure ()
  either (fail . T.unpack) pure result
  where
    insertOneEdge conn now edge = do
      stmt <- prepareTx conn "insert_runtime_edge"
        "INSERT INTO semantic_edges_runtime (ts, edge_from, edge_to, weight, co_occurrence, relation_type, domain, temporal_scope, verb, rationale, lineage, confidence, provenance, namespace) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
...
deleteRuntimeEdge :: QxFx0DB -> Text -> Text -> IO ()
deleteRuntimeEdge db from to = do
  result <- withDB (qdbPath db) $ \conn -> do
    stmt <- prepareTx conn "delete_runtime_edge"
      "DELETE FROM semantic_edges_runtime WHERE edge_from = ? AND edge_to = ?"
...
deleteRuntimeEdgeExact :: QxFx0DB -> Text -> Text -> RelationType -> EdgeNamespace -> IO ()
deleteRuntimeEdgeExact db from to rel ns = do
  result <- withDB (qdbPath db) $ \conn -> do
    stmt <- prepareTx conn "delete_runtime_edge_exact"
      "DELETE FROM semantic_edges_runtime WHERE edge_from = ? AND edge_to = ? AND relation_type = ? AND namespace = ?"
    bindTextOrFail stmt 1 from
    bindTextOrFail stmt 2 to
...
deleteRuntimeEdgeExactConn :: NSQL.Database -> Text -> Text -> RelationType -> EdgeNamespace -> IO ()
deleteRuntimeEdgeExactConn conn from to rel ns = do
...
loadRuntimeEdgeProjection :: QxFx0DB -> IO (Map (Text, Text) SemanticEdge)
loadRuntimeEdgeProjection db = do
  result <- withDB (qdbPath db) $ \conn -> do
    let sql = "SELECT edge_from, edge_to, weight, co_occurrence, relation_type, domain, temporal_scope, verb, rationale, lineage, confidence, provenance, namespace FROM semantic_edges_runtime ORDER BY id ASC"
    ...
    case mStmt of
      ...
      Right stmt -> do
        ...
        NSQL.finalize stmt
        pure (Right rows)
  either (fail . T.unpack) (either (fail . T.unpack) pure) result
...
loadAllRuntimeEdges :: QxFx0DB -> IO [SemanticEdge]
loadAllRuntimeEdges db = do
  result <- withDB (qdbPath db) $ \conn -> do
    let sql = "SELECT edge_from, edge_to, weight, co_occurrence, relation_type, domain, temporal_scope, verb, rationale, lineage, confidence, provenance, namespace FROM semantic_edges_runtime ORDER BY id ASC"
    mStmt <- NSQL.prepare conn sql
    case mStmt of
  ...
  where
    ...
    applyEdgeEvent from to event acc =
      ...
      in case leKind event of
        ...
        EdgeQuarantined -> M.delete key acc
